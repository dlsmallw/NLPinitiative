...
if HF_TOKEN is None:  # pragma: no cover
    set_key((dotenv_path), "HF_TOKEN", "")
    HF_TOKEN = get_key(dotenv_path, "HF_TOKEN")


def set_repo_id(repo_type: str, repo_id: str):  # pragma: no cover
    """
...
def set_base_url(url_type: str, base_url: str):  # pragma: no cover
    """
...
def set_token(token_type: str, token: str):  # pragma: no cover
    """
...
def main(
    ...
    """Update the repository IDs, base URLs in the pyproject.toml file and tokens in the .env file."""
    toml_edited = False
    ...
    token_param_list = [("HF_TOKEN", hf_token)]
    ...
    for url_type, base_url in url_param_list:
        if base_url is not None and len(base_url) > 0:
            if validators.url(base_url) is False:
                logger.error(
                    f"Invalid URL for {url_type}: {base_url}. Please provide a valid URL."
                )
            else:
